...
- 入力された値に合致するデータを `users.csv`の中から探すこと
  ...
  - 合致するデータが見つからなかった場合は、再度メールアドレスとパスワードを入力させること
    - 「既に登録されているメールアドレス、パスワードを入力してください」というメッセージを表示する
...
- 担当するユーザーコードが `users.csv`に登録されていない場合、AppExceptionをスローする
  ...
  - AppExceptionのメッセージを表示し、再度タスクコードの入力に戻る
...
- AppExceptionがスローされたらUI側でメッセージを表示し、再度タスクコードの入力に戻る
